...
RUN case ${distro} in \
    alpine) \
    apk update && apk add autoconf automake cmake curl g++ gcc git libgcc libtool leveldb-dev lmdb-dev libxml2-dev linux-headers make mariadb-dev musl musl-dev nodejs openssl-dev openssl-libs-static sqlite-dev yaml-dev yaml-static zlib-static && \
    ...
    ;; \
    fedora) \
    dnf update -y &&  dnf groupinstall -y 'Development Tools' && dnf install -y cmake leveldb-devel lmdb-devel openssl-static openssl-devel libxml2-devel libyaml-devel libsqlite3x-devel mariadb-devel \
    ;; \
    ubuntu) \
    apt update -y && \
    apt install -y autoconf bison build-essential curl gawk git libleveldb-dev libleveldb1d liblmdb-dev libmysqlclient-dev libnss3-dev libsnappy1v5 libsqlite3-dev libssl-dev libxml2-dev libyaml-dev pkg-config python3 rsync texinfo zlib1g-dev \
    ;; \
    *) \
...
RUN fpm  -s dir -t ${pkgfmt} -n gerbil-scheme -v ${GERBIL_VERSION} --description "Gerbil & Gambit scheme package for ${distro} Linux"  /opt/gerbil /opt/gambit
